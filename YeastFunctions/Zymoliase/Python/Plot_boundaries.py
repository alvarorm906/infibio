...
path_v = r'C:/Users/uib/Desktop/prueba_script_python/Drawing of B2.2_0775.tif_thresholded.tif_var.tif'
image_rgb = cv2.imread(path_v)
image_1 = rgb2gray(image_rgb)
image = (image_1*255).astype(np.uint8)
path_binary = r'C:/Users/uib/Desktop/prueba_script_python/B2.2_0775_mask.tif'
binary_img = cv2.imread(path_binary, cv2.IMREAD_GRAYSCALE)
...
"""
# Assuming `borders` is your list of borders
...
for i in range(len(borders)):
    ...
    # Compute Fourier coefficients for x and y separately
    x_ft = np.fft.fft(x)
    y_ft = np.fft.fft(y)
    
    # Combine magnitudes of Fourier coefficients for both x and y
    combined_ft = np.sqrt(np.abs(x_ft)**2 + np.abs(y_ft)**2)
    
    # Plot the combined coefficients
    plt.figure()
    plt.plot(combined_ft, label='Combined')  # Plot the magnitude of the combined Fourier coefficients
    plt.title(f'Combined Fourier Coefficients for Border {i}')
